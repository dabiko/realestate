...
@section('title')
    ...
    {{ config('app.name') }} | All {{$category_name->name}} Blog Post
    ...
    <section class="page-title centred" style="background-image: {{asset('frontend/assets/images/background/page-title-5.jpg')}};">
        <div class="auto-container">
            <div class="content-box clearfix">
                <h1>All {{$category_name->name}} Blog Post</h1>
                <ul class="bread-crumb clearfix">
                    <li><a href="{{route('home')}}">Home</a></li>
                    <li>{{$category_name->name}} Blog Post </li>
                </ul>
            </div>
        </div>
    </section>
            ...
            <div class="row clearfix">
                <div class="col-lg-8 col-md-12 col-sm-12 content-side">
                    <div class="blog-grid-content">
                        <div class="row clearfix">
                            @foreach($post_category_filter as $post)
                        ...
                        <div class="pagination-wrapper">
                            {{ $post_category_filter->links('vendor.pagination.custom') }}
                        </div>
                    </div>
                </div>
                            ...
                            <div class="widget-content">
                                <ul class="category-list clearfix">
                                    ...
                                    @foreach($categories as $category)
                                        ...
                                        @endphp
                                        <li><a href="{{route('blog-post-filter-category', ['category' => Crypt::encryptString($category->id)])}}">{{$category->name}}<span>({{count($category_post)}})</span></a></li>
                                    @endforeach
                                </ul>
                            </div>
                        </div>
